    ...
    >
      ...
      <div className="relative overflow-hidden bg-[#C8BBA6]" style={{ aspectRatio: "16/10" }}>
        <img
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1582468546235-9bf31e5bc4a1?w=800&h=500&fit=crop&auto=format";
          }}
        />
...
export function TourList() {
  ...
  const getFallbackTours = (): TourTemplate[] => {
    return TOURS.map((t: any) => ({
      ...
      badges: t.badges || [],
      is_active: true,
      default_duration_minutes: parseInt(t.duration) || 180,
    }));
  };
  ...
  const fetchTours = async (page = 1) => {
    try {
      ...
      try {
        ...
        const uniqueCategories = [...new Set(fallbackTours.map((t: TourTemplate) => t.category))].filter(Boolean) as string[];
        setCategories(uniqueCategories);
        
        // Set pagination for fallback
        setPagination({
          page: 1,
          limit: fallbackTours.length,
          total: fallbackTours.length,
          pages: 1,
        });
        setCurrentPage(1);
      } catch (fallbackErr) {
        ...
        <div className="mb-10 space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 text-[13px] text-[#7A6E60] pt-2 border-t border-border">
            <span>
              ...
              {usingFallback && (
                <span className="ml-2 text-[11px] text-[#B8952A] italic">(offline mode)</span>
              )}
            </span>
