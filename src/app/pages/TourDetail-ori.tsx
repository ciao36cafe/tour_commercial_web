    ...
    <div className="flex items-center gap-2">
      <div className="flex gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star key={i} size={14} className={i < Math.round(rating) ? "fill-[#B8952A] text-[#B8952A]" : "text-[#C8BBA6]"}/>
        ))}
      </div>
...
export function TourDetail() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-20">
        <div className="text-center">
          <p className="text-[#7A6E60] mb-4">Tour not found.</p>
          <button
            className="text-[#2D4A3E] text-sm underline"
            onClick={() => navigate("/")}
          >
            Return home
          </button>
  ...
  // IMPORTANT: Use priceFam directly from tour data, NOT priceGroup
  const priceFam = tour.priceFam || tour.priceGroup || tour.priceSingle; // Fallback if priceFam doesn't exist
  
  // Calculate totals for family trip
  ...
  const totalPeople = isFamilyTrip ? totalAdults + totalChildren : guests;
  
  // Family price: priceFam per family (per tuk-tuk)
  const totalPrice = isFamilyTrip 
    ? numberOfFamilies * priceFam // Each family pays the family package price
  ...
  // Handle family trip toggle
  const handleFamilyTripToggle = (checked: boolean) => {
    setIsFamilyTrip(checked);
    if (checked) {
      // When enabling family mode, keep current guest count as per-family values
      setNumberOfFamilies(1);
      setAdultsPerFamily(Math.min(guests, 4)); // Max 4 adults per family
      setChildrenPerFamily(0);
    } else {
      // When disabling, set guests to total people
      const totalPeople = numberOfFamilies * (adultsPerFamily + childrenPerFamily);
      setGuests(Math.max(1, totalPeople));
    }
  };

  // Validate children per family
  const getMaxChildrenPerFamily = () => 2; // Max 2 children per tuk-tuk
  ...
  return (
    <>
      {/* ── SEO meta (injected via title) ── */}
      {typeof document !== "undefined" && (document.title = `${tour.name} | Siam Journeys Bangkok`)}
      ...
      <div className="max-w-7xl mx-auto px-6 xl:px-12 py-12">
        <div className="lg:grid lg:grid-cols-[1fr_360px] gap-12 items-start">
          ...
          <div className="space-y-12">
            ...
            {/* Mobile: Tour Essentials (shown above main content on mobile) */}
            ...
            <section>
              ...
              <div className="divide-y divide-border">
                {tour.itinerary.map(({ time, title, description }, i) => (
                  <div key={i} className="py-4">
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <span className="text-[11px] tracking-[0.16em] text-[#B8952A] w-[68px] flex-shrink-0">{time}</span>
                        <span
                ...
                <div className={`grid gap-3 ${tour.galleryImgs.length >= 3 ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-2"}`}>
                  {tour.galleryImgs.map((src, i) => (
                    <div key={i} className={`overflow-hidden group bg-[#C8BBA6] ${i === 0 && tour.galleryImgs.length >= 3 ? "sm:col-span-2 sm:row-span-2" : ""}`} style={{ aspectRatio: i === 0 && tour.galleryImgs.length >= 3 ? "1/1" : "4/3" }}>
                      <img src={src} alt={`${tour.name} gallery image ${i + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                    </div>
                  ))}
                </div>
              </section>
            ...
            {/* Tour Essentials — full detail, desktop-readable */}
...
function MobileEssentialsSummary({ essentials }: { essentials: { dressCode: string; fitness: string; agePolicy: string; prep: string[] } }) {
  return (
